...
export default function StablecoinBridge() {
	...
	useEffect(() => {
		...
		if (!activeMinter) {
			setErrorBridge("The bridge module has not yet completed the governance process.");
		} else if (horizon.getTime() < now && direction) {
			setErrorBridge(`Bridge module has expired on ${horizon.toDateString()}`);
		} else if (timeRemaining < thirtyDaysMs && timeRemaining > 0 && direction) {
			const daysRemaining = Math.ceil(timeRemaining / (24 * 60 * 60 * 1000));
			setErrorBridge(
				`Warning: Bridge module expires in ${daysRemaining} day${daysRemaining === 1 ? "" : "s"} (${horizon.toDateString()})`
			);
		} else {
			setErrorBridge("");
		}
	...
	useEffect(() => {
		if (amount > fromBalance) {
			...
			setError(`Not enough ${toSymbol} available to convert.`);
		} else {
			setError("");
		}
	...
	const handleApprove = async () => {
		try {
			...
			const toastContent = [
				...
				{ title: "Spender: ", value: shortenAddress(bridge) },
				{ title: "Transaction:", hash: approveWriteHash },
			];

			await toast.promise(waitForTransactionReceipt(WAGMI_CONFIG, { hash: approveWriteHash, confirmations: 1 }), {
				pending: { render: <TxToast title={`Approving ${fromSymbol}`} rows={toastContent} /> },
				success: { render: <TxToast title={`Successfully Approved ${fromSymbol}`} rows={toastContent} /> },
			});
		} catch (error) {
	...
	const handleMint = async () => {
		try {
			...
			const toastContent = [
				{ title: `${fromSymbol} Amount: `, value: formatBigInt(amount) + " " + fromSymbol },
				{ title: `${toSymbol} Amount: `, value: formatBigInt(amount) + " " + toSymbol },
			...
			await toast.promise(waitForTransactionReceipt(WAGMI_CONFIG, { hash: mintWriteHash, confirmations: 1 }), {
				pending: { render: <TxToast title={`Converting ${fromSymbol} to ${toSymbol}`} rows={toastContent} /> },
				success: { render: <TxToast title={`Successfully Converted ${fromSymbol} to ${toSymbol}`} rows={toastContent} /> },
			});
		} catch (error) {
			toast.error(renderErrorTxToast(error));
		} finally {
	...
	return (
		<>
			<Head>
				<title>Stablecoin Bridge | ZCHF Desk</title>
			</Head>

			<div className="md:mt-8">
				<section className="mx-auto max-w-2xl sm:px-8">
					<AppCard>
						<div className="mt-4 text-lg font-bold text-center">Stablecoin Bridge: {swapStats.otherSymbol} and ZCHF</div>

						<div className="mt-8">
							The <AppLink className="" label="bridge module" href={bridgeUrl} external={true} /> enables 1:1 conversion between
							other Swiss franc stablecoins and ZCHF, up to certain limits. Currently,{" "}
						...
						<div className="py-4 text-center z-0">
							<AppButton className={`h-10 rounded-full`} width="w-10" onClick={() => setDirection(!direction)}>
							...
							<GuardSupportedChain chain={mainnet}>
								{direction ? (
									amount > swapStats.otherUserAllowance ? (
										...
										<AppButton disabled={amount == 0n || !activeMinter || !!error} isLoading={isMinting} onClick={() => handleMint()}>
											Convert
										</AppButton>
									)
								) : (
